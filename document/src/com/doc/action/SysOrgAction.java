...
public class SysOrgAction  extends ActionSupport{
	...
	public String querySysOrgList() throws Exception{
    	...
    	try {
			...
			List listcount = sysOrgService.querySysOrgList(param);
			jsonMap.put("total",listcount.size() );// total  存放总记录数
			
			param.put("begin", String.valueOf(start));
			param.put("pageSize", String.valueOf(pageSize));
			
			List list = sysOrgService.querySysOrgList(param);
	...
	public String changeRows() throws Exception{
		try {
			...
			sysOrgService.changeRowsList(map);
			jsonMap = new HashMap();
			jsonMap.put("info", 1);
		} catch (Exception e) {
			e.printStackTrace();
