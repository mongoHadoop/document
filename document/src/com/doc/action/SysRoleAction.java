...
public class SysRoleAction  extends ActionSupport{
	...
	public String querySysRoleList() throws Exception{
    	...
    	try {
			...
			List listcount = sysRoleService.querySysRoleList(param);
			jsonMap.put("total",listcount.size() );// total  存放总记录数
			
			param.put("begin", String.valueOf(start));
			param.put("pageSize", String.valueOf(pageSize));
			
			List list = sysRoleService.querySysRoleList(param);
	...
	public String changeRows() throws Exception{
		try {
			...
			sysRoleService.changeRowsList(map);
			jsonMap = new HashMap();
			jsonMap.put("info", 1);
		} catch (Exception e) {
			e.printStackTrace();
